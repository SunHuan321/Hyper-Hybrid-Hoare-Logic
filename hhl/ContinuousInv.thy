...
theorem Valid_inv:
  ...
  assumes "\<forall>x. ((\<lambda>v. inv (vec2state v)) has_derivative g' x) (at x within UNIV)"
    ...
    shows "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. inv s = r \<and> tr = tra \<and> b s}
       Cont ode b
...
theorem Valid_ode_supp:
  ...
  shows "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. supp s \<subseteq> VS}
     Cont (ODE ode) b
...
theorem Valid_inv_barrier_s_tr_le:
  ...
  shows "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. inv s \<le> 0 \<and> b s \<and> P tr}
     Cont (ODE ode) b
...
theorem DC:
  assumes "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. init s \<and> P tr \<and> b s}
     Cont ode b
    {\<lambda>s tr. (P @\<^sub>t ode_inv_assn c) tr}"
      and "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. init s \<and> P tr \<and> (\<lambda>s. b s \<and> c s) s}
     Cont ode (\<lambda>s. b s \<and> c s)
    {\<lambda>s tr. (P @\<^sub>t ode_inv_assn d) tr}"
    shows "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. init s \<and> P tr \<and> b s}
     Cont ode b
    {\<lambda>s tr. (P @\<^sub>t ode_inv_assn d) tr}"
unfolding Valid_def
  apply (auto elim!: contE)
