...
theorem Valid_interrupt:
  assumes "\<And>i. i < length cs \<Longrightarrow>
    case cs ! i of
      (ch[!]e, p2) \<Rightarrow>
        (\<exists>Q. \<Turnstile>\<^sub>H\<^sub>L {Q} p2 {R} \<and>
  ...
  shows "\<Turnstile>\<^sub>H\<^sub>L {P} Interrupt ode b cs {R}"
proof -
...
theorem Valid_interrupt_In:
  assumes "\<Turnstile>\<^sub>H\<^sub>L {Q} p {R}"
...
theorem Valid_ode':
  "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. \<forall>s'. (ODE\<^sub>t s ode b s' @- Q s') tr}
       Cont ode b
      {Q}"
proof -
  have 1: "Q s tr"
...
theorem Valid_ode_sp_st:
  "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s t. s = s0 \<and> Q s t}
        Cont ode b
...
theorem Valid_interrupt':
  ...
  shows "\<Turnstile>\<^sub>H\<^sub>L {P} Interrupt ode b cs {R}"
proof -
  have 1: "\<exists>Q. \<Turnstile>\<^sub>H\<^sub>L {Q} p {R} \<and>
           (P \<Longrightarrow>\<^sub>A (\<lambda>s tr. Q s (tr @ [OutBlock ch (e s)]))) \<and>
  ...
  proof -
    from assms(1) obtain Q where
      Q: "\<Turnstile>\<^sub>H\<^sub>L {Q} p {R} \<and> (P \<Longrightarrow>\<^sub>A (\<lambda>s tr. \<forall>s'. (ODEout\<^sub>t s ode b s' ch e (rdy_of_echoice cs) @- Q s') tr))"
      using * by fastforce
    show ?thesis
      apply (rule exI[where x=Q])
      using Q ode_out_assn.intros by (auto simp add: entails_def magic_wand_assn_def)
  qed
  have 2: "\<exists>Q. \<Turnstile>\<^sub>H\<^sub>L {Q} p {R} \<and>
           (P \<Longrightarrow>\<^sub>A (\<lambda>s tr. \<forall>v. Q (s(var := v)) (tr @ [InBlock ch v]))) \<and>
           (P \<Longrightarrow>\<^sub>A
            (\<lambda>s tr.
                \<forall>d>0. \<forall>p. ODEsol ode p d \<longrightarrow>
  ...
  proof -
    from assms(1) obtain Q where
      Q: "\<Turnstile>\<^sub>H\<^sub>L {Q} p {R} \<and> (P \<Longrightarrow>\<^sub>A (\<lambda>s tr. \<forall>s'. (ODEin\<^sub>t s ode b s' ch var (rdy_of_echoice cs) @- Q s') tr))"
      using * by fastforce
    show ?thesis
      apply (rule exI[where x=Q])
...
theorem Valid_interrupt_sp:
  assumes "\<And>i. i < length cs \<Longrightarrow>
    case cs ! i of
      (ch[!]e, p2) \<Rightarrow>
        \<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. (P st @\<^sub>t ODEout\<^sub>t st ode b s ch e (rdy_of_echoice cs)) tr} p2 {Q}
    | (ch[?]var, p2) \<Rightarrow>
        \<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. (P st @\<^sub>t ODEin\<^sub>t st ode b s ch var (rdy_of_echoice cs)) tr} p2 {Q}"
...
theorem Valid_interrupt_In_sp:
  assumes "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. (P st @\<^sub>t ODEin\<^sub>t st ode b s ch var ({}, {ch})) tr} p {Q}"
  shows "\<Turnstile>\<^sub>H\<^sub>L
    {\<lambda>s tr. s = st \<and> P s tr}
      Interrupt ode b [(ch[?]var, p)]
...
theorem Valid_interrupt_Out_sp:
  assumes "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. (P st @\<^sub>t ODEout\<^sub>t st ode b s ch e ({ch}, {})) tr} p {Q}"
  shows "\<Turnstile>\<^sub>H\<^sub>L
    {\<lambda>s tr. s = st \<and> P s tr}
      Interrupt ode b [(ch[!]e, p)]
    {\<lambda>s tr. Q s tr \<or> (P st @\<^sub>t (ODErdy\<^sub>t st ode b s ({ch}, {}))) tr}"
...
theorem Valid_interrupt_Out_sp2:
  assumes "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. \<exists>st'. s = st' \<and> supp st' \<subseteq> VS \<and>
...
proof -
  ...
  show ?thesis
    ...
    subgoal for d p2 rdy1 rdy2
      ...
      using main[of d p2] apply blast
      using main[of d p2] apply (meson WaitBlk_eqI)
      using main[of d p2] by presburger
    done
qed
...
theorem Valid_ode_out_unique_solution:
  assumes
    "ODEsolInf ode p" "\<forall>t\<ge>0. b (p t)" "p 0 = st"
    "local_lipschitz {- 1<..} UNIV (\<lambda>(t::real) v. ODE2Vec ode (vec2state v))"
    "\<Turnstile>\<^sub>H\<^sub>L {\<lambda>s tr. \<exists>d. s = p d \<and> (P st @\<^sub>t WaitOut\<^sub>t d p ch e ({ch}, {})) tr} p2 {Q}"
  shows "\<Turnstile>\<^sub>H\<^sub>L
    {\<lambda>s tr. s = st \<and> P s tr}
